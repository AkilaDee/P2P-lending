...
export default function LendRequests() {
  ...
  const handleConfirm = async () => {
    if (selectedLendRequestId) {
      handlePaymentSuccess();
    } else {
      console.error("No lend request ID available.");
    }
    setOpenConfirm(false);
  };
  ...
  const handlePaymentSuccess = () => {
    ...
    console.log("oooooo",selectedLendRequestId, userId);
    axios.post(`${backendUrl}/users/lendrequests/pay`, { lendRequestId: selectedLendRequestId })
      .then((response) => {
              ...
              <Table>
                ...
                <TableBody>
                  {rows.filter((row) => {
                    ...
                    return false;  // Always provide a fallback case
                  }).map((row, id) => (
                    <TableRow key={id}>
